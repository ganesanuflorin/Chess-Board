...
// Primeste tabla ca parametru si obtine urmatoarea mutare
char* Book::getNextMove (ChessBoard tabla, COLOR culoare)
{
    int i,j,poz;
    INITIALIZE();
    if (ERROR) {
        fprintf(debug_file,"Missing obliged file(s) RANDOM1.BIN and/or RANDOM2.BIN");
        exit(1);
    }
...
void Book::INITIALIZE()
{
    ...
    for (x=0; x<1120; x++) {
        fread(&RANDOM1[x],4,1,fp1); fread(&RANDOM2[x],4,1,fp2);
    }
...
void Book::FIND_OPENING() {
    ...
    RND=clock();                                // initializeaza randomizatorul
    x=rand()%9999; rnd=RND+x;
    RND=rnd;                                    // info debug
    AZ=0;                                       // numar de mutari gasite in book
    boekfase=1;                // nu a fost selectat niciun book -> selecteaza book-ul principal
